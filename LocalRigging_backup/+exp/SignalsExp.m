classdef SignalsExp < handle
  ...
  properties
    ...
    WaterKey = KbName('space'); % 170803 AP - enable keyboard water
    ...
    LickDetector % AP 170803 made this an signal below (what was it before??)
...
%     Audio = aud.AudioRegistry
  ...
  methods
    function obj = SignalsExp(paramStruct)
      ...
      obj.Inputs.lever_r = net.origin('lever_r'); % AP 170629
      obj.Inputs.lickDetector = net.origin('lickDetector'); % AP 170803
...
%       obj.Data.stimWindowUpdateLags = zeros(60*60*60*2, 1);
    ...
    function useRig(obj, rig)
      ...
      obj.lever_r = rig.lever_r; % AP 170629

      if isfield(rig, 'lickDetector')
        obj.LickDetector = rig.lickDetector; % changing upper/lower case is really dumb
      end
...
%         tic
        ...
        %%% ADDED LEVER AP 170629
        lever_r_value = readAbsolutePosition(obj.lever_r);
        post(obj.Inputs.lever_r, lever_r_value);
        %%%
        
         %%% ADDED LICK AP 170803
        lick_value = readAbsolutePosition(obj.LickDetector);
        post(obj.Inputs.lickDetector, lick_value);
        %%%
...
%           toc;
    ...
    function checkInput(obj)
      ...
      if pressed
        ...
        elseif any(keysPressed(obj.WaterKey)) %170803 AP - keyboard water
            rewardId = 1;
            def = [obj.DaqController.SignalGenerators(rewardId).DefaultCommand];
            obj.DaqController.command(def);
        end
      end
    end
